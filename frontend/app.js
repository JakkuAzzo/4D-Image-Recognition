...
async function refreshOSINT() {
    ...
    try {
        // Use test user for OSINT data
        const testUserId = 'test_user_jane_001';
        const response = await fetch(`/osint-data?user_id=${testUserId}&source=${sourceFilter}`);
...
function displayOSINTResults(data, filter) {
    ...
    if (data.sources) {
        ...
        if (filter === 'all' || filter === 'social') {
            if (sources.social) {
                cards.push({
                    ...
                    status: sources.social.confidence > 0.8 ? 'success' : 'warning',
                    content: `Found ${sources.social.profiles_found} profiles across ${sources.social.platforms.join(', ')}. Confidence: ${Math.round(sources.social.confidence * 100)}%`,
                    category: 'social'
                });
            }
        }
        ...
        if (filter === 'all' || filter === 'public') {
            if (sources.public) {
                cards.push({
                    ...
                    status: sources.public.confidence > 0.7 ? 'success' : 'warning',
                    content: `${sources.public.matches} records found: ${sources.public.records.join(', ')}. Confidence: ${Math.round(sources.public.confidence * 100)}%`,
                    category: 'public'
                });
            }
        }
        ...
        if (filter === 'all' || filter === 'financial') {
            if (sources.financial) {
                cards.push({
                    ...
                    status: sources.financial.confidence > 0.6 ? 'success' : 'warning',
                    content: `${sources.financial.credit_check} available. ${sources.financial.bankruptcy_records}. Confidence: ${Math.round(sources.financial.confidence * 100)}%`,
                    category: 'financial'
                });
            }
        }
        ...
        if (filter === 'all' || filter === 'professional') {
            if (sources.professional) {
                cards.push({
                    title: 'Professional Verification',
                    status: sources.professional.confidence > 0.9 ? 'success' : 'warning',
                    content: `${sources.professional.employment} at ${sources.professional.company}. Confidence: ${Math.round(sources.professional.confidence * 100)}%`,
                    category: 'professional'
                });
            }
        }
        
        if (filter === 'all' || filter === 'biometric') {
            if (sources.biometric) {
                cards.push({
                    title: 'Biometric Cross-Reference',
                    status: sources.biometric.confidence > 0.9 ? 'success' : 'warning',
                    content: `${sources.biometric.facial_matches} matches across ${sources.biometric.databases_searched} databases. Confidence: ${Math.round(sources.biometric.confidence * 100)}%`,
                    category: 'biometric'
                });
            }
        }
    ...
    // Render the cards
    if (cards.length === 0) {
        osintGrid.innerHTML = '<div class="osint-card"><h4><span class="status-indicator status-info"></span>No Data</h4><p>No OSINT data available for the selected filter.</p></div>';
        return;
    }
    ...
    osintGrid.innerHTML = cardElements;
}
...
async function verifyId() {
    ...
    const selfieFile = document.getElementById('selfie_image').files[0];
...
async function ingestScan() {
    const userId = document.getElementById('ing_user_id').value;
    const files = document.getElementById('ing_files').files;
    ...
    try {
        // Send as form data, not query parameter
